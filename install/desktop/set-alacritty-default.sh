...
if [ "$OS_TYPE" = "ubuntu" ]; then
  ...
  # Add Alacritty to Nautilus contextual menu
...
class OpenTerminalExtension(GObject.GObject, Nautilus.MenuProvider):
    ...
    def menu_background_activate_cb(self, menu: Nautilus.MenuItem, file: Nautilus.FileInfo) -> None:
        self._open_terminal(file)
...
TECHNICALLYNOTACONFIGSOHEREDOCCEDITIS
  ...
  echo "Setting default terminal not supported on macOS. Configure manually in Terminal or iTerm2."
fi
